...
def main():
    ...
    if not autornaseq_dir.exists():
        raise FileNotFoundError(f"Directory not found: {autornaseq_dir}")
    
    nfcore = df_from_quantsf_files(nfcore_dir, suffix="nfcore")
    reproduction = df_from_quantsf_files(autornaseq_dir, suffix="autornaseq")
