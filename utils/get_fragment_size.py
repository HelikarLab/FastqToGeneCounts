...
def main():
    print("\n")
    num_lines = sum(1 for i in open(snakemake.config["BED_FILE"]))
    with open(snakemake.output[0], "w") as o_stream:
        write_header(o_stream)
        for i, line in enumerate(fragment_size(
                bedfile=snakemake.config["BED_FILE"],
                samfile=snakemake.input.bam,
                qcut=30,
                ncut=3
        )):
